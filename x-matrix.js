...
function drawClickers(canvas, matrix) {
  ...
  canvas
    ...
    .data((d) => d)
    .enter()
    .append("g") // foreach patch
    .attr("transform", (d) =>
      translate(2 * d.originNorth.x, 2 * d.originNorth.y)
    )
    .selectAll("_")
    .data((d) => d.patch)
    .enter()
    .append("g") // foreach alley
    .attr("transform", (d, i) => translate(2 * i, 2 * i))
    .selectAll("_")
    .data((d) => d)
    .enter()
    .append("circle") // foreach clicker
    .attr("class", "dot")
    .attr("r", ".6")
    .attr("cx", (d, i) => -2 * i)
    .attr("cy", (d, i) => +2 * i)
    .on("click", function (d) {
      d3.select(this).attr(
        "class",
        "dot " + binding[++d.clicks % binding.length]
      );
    });
}
...
function gridLines(rows, leg, arm) {
  let lines = "";
  for (h = 0; h <= rows; h++) {
    lines += M(arm + 2 * h, 2 * h) + l(-arm, 0) + l(-leg, leg);
  }
  return lines;
}
